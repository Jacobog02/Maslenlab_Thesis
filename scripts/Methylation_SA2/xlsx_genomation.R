...
option_list = list(
  ...
  make_option(c("-x", "--xlsx"), type="logical", default=TRUE,
              help="Logical for optional xslx output", metavar="character")
); 
...
if (is.null(opt$xlsx)){
  print_help(opt_parser)
  stop("XLSX is logical default TRUE.n", call.=FALSE)
}
...
bco3 <- zero_check %>% group_by(region_id) %>% slice_max(order_by = region_id,n=1, with_ties = F) %>% select(-n) %>% ungroup()
