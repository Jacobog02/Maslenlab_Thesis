...
if [ ! -f $wrkdir/int.$INFILE.bed ]; then

    $exa_sam view -c $sourcedir/reads/srt.$INFILE.bam > $wrkdir/int.$INFILE.bed

    bedtools intersect -v \
    -abam $sourcedir/reads/srt.$INFILE.bam \
    -b $wrkdir/srt-${probe_name} -bed | awk '{print $1, $2, $3, "*"}' >> $wrkdir/int.$INFILE.bed
fi 
